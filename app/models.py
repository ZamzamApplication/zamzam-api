...
DEFAULT_ABSENT_STATUS = AttendanceStatus.absent.value
...
DEFAULT_EXCEL_EXPORT_TEMPLATES = {
    "attendance": {
        ...
        "columns": [
            {"id": "serial", "label": "م", "enabled": False, "custom": False, "width": 6, "header_font_size": 12},
            {"id": "student", "label": "الطالب", "enabled": True, "custom": False, "width": 24, "header_font_size": 12},
            {"id": "sheikh", "label": "الشيخ", "enabled": True, "custom": False, "width": 20, "header_font_size": 12},
            {"id": "subscription_amount", "label": "مبلغ الاشتراك", "enabled": True, "custom": False, "width": 16, "header_font_size": 12},
            {
                "id": "memorization", "label": "الحفظ", "enabled": True, "custom": False,
                "width": 18, "header_font_size": 12,
                "subcolumns": [
            ...
            {"id": "attendance", "label": "الحضور", "enabled": True, "custom": False, "width": 18, "header_font_size": 12},
        ],
    },
    "statistics": {
        "header_font_family": "Arial",
        ...
        "columns": [
            ...
            {"id": "student", "label": "الطالب", "enabled": True, "custom": False, "width": 24, "header_font_size": 12},
            {"id": "sheikh", "label": "الشيخ", "enabled": True, "custom": False, "width": 20, "header_font_size": 12},
            {"id": "sessions", "label": "إجمالي الحلقات", "enabled": True, "custom": False, "width": 16, "header_font_size": 12},
            {"id": "statuses", "label": "حالات الحضور", "enabled": True, "custom": False, "width": 14, "header_font_size": 12},
            {"id": "rate", "label": "نسبة الحضور", "enabled": True, "custom": False, "width": 16, "header_font_size": 12},
        ],
    },
    "progress": {
        "header_font_family": "Arial",
        ...
        "columns": [
            {"id": "serial", "label": "م", "enabled": False, "custom": False, "width": 6, "header_font_size": 12},
            {"id": "student", "label": "الطالب", "enabled": True, "custom": False, "width": 24, "header_font_size": 12},
            {"id": "entries", "label": "عدد سجلات المتابعة", "enabled": True, "custom": False, "width": 20, "header_font_size": 12},
            {"id": "quality", "label": "متوسط التقييم", "enabled": True, "custom": False, "width": 18, "header_font_size": 12},
            {"id": "mistakes", "label": "إجمالي الأخطاء", "enabled": True, "custom": False, "width": 18, "header_font_size": 12},
            {"id": "latestRange", "label": "آخر مقدار", "enabled": True, "custom": False, "width": 28, "header_font_size": 12},
        ],
    },
}
...
def excel_export_template_options(tahfiz: "Tahfiz") -> dict:
    ...
    for key, default in DEFAULT_EXCEL_EXPORT_TEMPLATES.items():
        ...
        fallback_header_size = int(template.get("header_font_size") or default["header_font_size"])
        for column in template["columns"]:
            if isinstance(column, dict):
                column.pop("header_font_family", None)
                if not isinstance(column.get("header_font_size"), int):
                    column["header_font_size"] = fallback_header_size
        configured_ids = {
            column.get("id")
            for column in template["columns"]
            if isinstance(column, dict)
...
def absent_status_option(tahfiz: "Tahfiz") -> str:
    statuses = attendance_status_options(tahfiz)
    configured = (getattr(tahfiz, "absent_status", None) or "").strip()
    if configured in statuses:
        return configured
    if DEFAULT_ABSENT_STATUS in statuses:
        return DEFAULT_ABSENT_STATUS
    colors = attendance_status_color_options(tahfiz)
    slate_status = next((status for status in statuses if colors.get(status) == "slate"), None)
    if slate_status:
        return slate_status
    present_status = present_status_option(tahfiz)
    return next((status for status in statuses if status != present_status), statuses[0])
...
class Tahfiz(Base):
    ...
    attendance_sheikh_selection_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    ...
    absent_status: Mapped[str] = mapped_column(
        String(50),
